...
# Obtain and store the list of instruments in the symbol_info dict
    ...
    def on_price_update(self, msg):
        ...
        try:
            ...
            #print(p_up)
            self.symbols[symbol] = self.symbols.get(symbol, PriceUpdate(p_up, symbol_info=symbol_info))
            #print(md['Rates'])
            #self.last_price = {"Time": pd.to_datetime(md['Updated'], unit = "ms"), "Bid": md["Rates"][0], "Ask":md["Rates"][1]}
            #self.last_bar_sec = pd.to_datetime('00:'+((str(pd.to_datetime(md['Updated'], unit = "ms")))[14:26]))
            #2022-04-27 14:38:45.592000
            #print((self.last_bar_sec))
            #self.tickdata = self.tickdata.append(dict_price, ignore_index=True)
            print(self.symbols[symbol].ask)
